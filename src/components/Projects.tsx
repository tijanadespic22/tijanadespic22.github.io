    ...
    <div className={'mx-auto max-w-7xl px-4 sm:px-6 lg:px-8'}>
      ...
      <div className={'mb-12'}>
        <Link
          href={'/'}
          className={
            'group mb-8 inline-flex items-center gap-1 text-sm text-gray-500 transition hover:text-gray-900 dark:text-gray-400 dark:hover:text-white'
          }>
          <ArrowIcon
            width={14}
            height={14}
      ...
      <div className={'grid grid-cols-1 gap-6 lg:grid-cols-3'}>
        {caseStudies.map((project, index) => (
          <Link
            ...
            className={
              'group flex flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white transition-all duration-300 hover:shadow-xl dark:border-gray-700 dark:bg-gray-800'
            }>
            ...
            <div className={'flex flex-1 flex-col gap-2 p-5'}>
              <div className={'flex items-center justify-between'}>
                <span
                  className={
                    'rounded-full bg-gray-100 px-2.5 py-1 text-xs text-gray-600 dark:bg-gray-700 dark:text-gray-400'
                  }>
                  {t(project.category)}
                </span>
              </div>
              ...
              <div className={'flex flex-wrap gap-1.5 pt-1'}>
                {project.technologies.slice(0, 3).map(tech => (
                  <span
                    key={tech}
                    className={
                      'rounded-full border border-blue-100 bg-blue-50 px-2 py-0.5 text-xs text-blue-600 dark:border-blue-900 dark:bg-blue-950/40 dark:text-blue-400'
                    }>
                    {t(tech)}
                  </span>
                ))}
                {project.technologies.length > 3 ? (
                  <span
                    className={
                      'rounded-full bg-gray-50 px-2 py-0.5 text-xs text-gray-400 dark:bg-gray-700'
                    }>
                    {'+'}
                    {project.technologies.length - 3}
                  </span>
      ...
      <div
        className={
          'mt-12 flex items-start gap-4 rounded-2xl border border-gray-200 bg-gray-50 p-6 dark:border-gray-700 dark:bg-gray-800'
        }>
      ...
      <div className={'mt-20 text-center'}>
        ...
        <Link
          href={'/#contact'}
          className={
            'group inline-flex items-center gap-2 rounded-full bg-linear-to-r from-blue-600 to-purple-600 px-8 py-4 font-medium text-white transition hover:opacity-90'
          }>
          {t('Kontaktiraj me')}
          <ArrowIcon
            width={18}
